...
describe("snipai.statusline.status", function()
  ...
  it("returns empty for a buffer whose file is not touched by any active job", function()
    setup()
    assert.equals("", statusline.status(0))
  end)

  it("returns the indicator when an active job has touched this buffer's file", function()
    local runner = setup()
    snipai.trigger("no_params")
    runner.spawns[1].on_event({
      kind = "tool_use",
      tool = "Edit",
      input = { file_path = "/proj/a.lua" },
    })

    local buf = vim.api.nvim_create_buf(false, false)
    vim.api.nvim_buf_set_name(buf, "/proj/a.lua")
    assert.equals("⟳ snipai", statusline.status(buf))

    -- finishing the job drops it from the active list → status clears
